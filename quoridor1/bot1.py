    ...
    def choose_move(self, gamestate):
        ...
        for i in range(calcs):
            ...
            while curr.fully_expanded:
                curr = curr.select_child(curr.visits, 1.3)
            ...
            print(f"[{bar_character*int((i/calcs)*bar_length)}{(bar_length-int((i/calcs)*bar_length))*' '}] {i}/{calcs} ---- win_confidence: {100*best_child.wins/max(best_child.visits,1):.2f}%       best: {best_child.reached_by}, {best_child.wins/best_child.visits:.2f} {chain}          ", end='\r')
